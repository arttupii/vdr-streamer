...
echo nice -n 10 cvlc -vvv `ls -L $INPUT/*.ts` vlc://quit  --sout-keep --sout-x264-quiet -v 0 --sub-language fin,fi,suomi --audio-language fin,fi,suomi --sout '#gather:transcode{vcodec=h264, scale=1,vb=1500,acodec=mpga,ab=128, soverlay,deinterlace, venc=x264{keyint=120,bpyramid,chroma-me,me-range=8,ref=1,ratetol=1.0,8x8dct,mixed-refs,direct=auto,direct-8x8=-1,non-deterministic,scenecut=50}}:standard{access=file,mux=mp4,dst='"$TARGET_FOLDER"/"$TARGET_FILE.tmp"'}' 
